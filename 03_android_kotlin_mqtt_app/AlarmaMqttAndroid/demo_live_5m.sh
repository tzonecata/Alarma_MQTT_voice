...
timeout "$((DURATION_SECONDS + 30))" "$SUB" --url "$BROKER_URL" -t "$TOPIC" -q 1 || true

echo

echo "[03] Subscriber demo window finished."
exec bash
EOF
...
echo "======================================================"
echo "DEMO RUN: $RUN_ID"
echo "TAB [04] RANDOM_ON_OFF_PUBLISHER_5M"
echo "======================================================"
...
echo "[04] Total sent: $count | ON=$count_on | OFF=$count_off"
exec bash
EOF
...
if [[ "$HEADLESS" == "1" ]]; then
  ...
  launch_headless "04_RANDOM_ON_OFF_PUBLISHER_5M" "$WORK_DIR/tab04_random_pub.sh"
  ...
  sleep 0.2
  launch_window "[${RUN_ID}] 04_RANDOM_ON_OFF_PUBLISHER_5M" "$WORK_DIR/tab04_random_pub.sh"
  sleep 0.2
